#!/usr/bin/env python3
"""Local RAG memory for liderdent Cursor agents. Stdlib only (SQLite FTS5)."""
...
def collect_sources() -> list[tuple[Path, str]]:
    ...
    if docs_dir.exists():
        for path in sorted(docs_dir.rglob("*.md")):
            sources.append((path, "docs"))

    seov_dir = PROJECT_ROOT / "seov"
    if seov_dir.exists():
        for path in sorted(seov_dir.rglob("*.md")):
            if ".venv" in path.parts:
                continue
            sources.append((path, "seov"))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="liderdent agent RAG memory")
    ...
    p_remember.add_argument(
        "--tag",
        default="note",
        choices=("note", "fact", "decision", "deploy", "bugfix", "theme"),
    )
